...
class Employee(models.Model):
    ...
    def get_hours_worked(self, today):
        ...
        timesheet_line_ids = self.env['hr.analytic.timesheet'].search(args=[('user_id', '=', self.user_id.id),
                                                                            ...
                                                                            ('date', '<=', today_str)])
        '''
        timesheet_line_ids = self.env['hr_timesheet.sheet.line'].search(args=[('employee_id', '=', self.id),
                                                                            ('date', '>=', self.hour_balance_start),
                                                                            ('date', '<=', today_str)])
    ...
    def _get_hour_balance(self):
        ...
        if self.weekly_working_time != 'hour_worker' and self.hour_balance_start:
            ...
            date_to_check = datetime.datetime.strptime(str(self.hour_balance_start), "%Y-%m-%d").date()
    ...
    hour_balance_start = fields.Date(string='Hour Balance Start Date', help='Date from which onwards the hour balance is calculated. Set this as the date when the user started filling out timesheets.')
    hour_balance = fields.Float(string='Hour Balance', compute=_get_hour_balance, store=True)

    # Fill the existing m2o relation between employees and timesheets so that it can be used in api.depends
    timesheet_ids = fields.One2many(comodel_name='hr_timesheet.sheet', inverse_name='employee_id', string='Timesheets')

    # A helper field that is used to restrict that the user can only see their own hour balance in the employee form view
    show_balance = fields.Boolean(string='Show Hour Balance', compute=_get_show_balance)
